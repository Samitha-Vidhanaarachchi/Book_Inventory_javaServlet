...
<%@page import="Service.MPurchaseService"%>
<%@page import="Service.MPurchaseServiceImp"%>
<%@page import="Model.MaterialPurchase"%>
<%@page import="java.util.ArrayList" %>
<!DOCTYPE html>
<html>
<head>
 <title>Material Purchase View </title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
...
<br>
                     <ul class="nav nav-pills nav-pills-icons justify-content-center" role="tablist">                        
                          <li class="nav-item">
                            <a class="nav-link active " href="materialPurchaseView.jsp" role="tab" >
                              <i class="material-icons">local_shipping</i>Material Purchases
                            </a>
                          </li>

                          <li class="nav-item">
                            <a class="nav-link" href="addMaterialPurchase.jsp" role="tab" >
                              <i class="material-icons">add</i> Add Material Purchase
                            </a>
                          </li>
                          
                          <li class="nav-item">
                            <a class="nav-link" href="#" role="tab" >
                              <i class="material-icons">edit</i> Edit Material Purchase
                            </a>
                          </li>
      ...
      <tr style= "background-color: rgb(150, 70, 150);font-size: 20px; color:white; font-weight:bold;">
       <!--   <td >Supplier ID</td> -->
        ...
        <td style="font-size: 15px;">Supplier</td>
        <td style="font-size: 15px;">Purchased Date</td>
        <td style="font-size: 15px;">Amount (KG/L)</td>
        <td style="font-size: 15px;" >Action</td>
     </tr>
     </thead>
     
      <%
     	MPurchaseService iMpur = new MPurchaseServiceImp(); 
     	ArrayList<MaterialPurchase> list = iMpur.listMpurchase();
     	
     	for( MaterialPurchase mp1 : list){
     %>
     <tbody>
     <tr>
        <td style="font-size: 12px; "><%=mp1.getId() %></td>
        <td style="font-size: 12px;"><%=mp1.getmName() %></td>
        <td style="font-size: 12px;"><%=mp1.getsId() %></td>
        <td style="font-size: 12px;"><%=mp1.getDate() %></td>
        <td style="font-size: 12px;"><%=mp1.getAmount() %></td>
        <td>
        	
        	<form action = "GetIdmPurchaseServlet" method = "post">
                    	<input type="hidden" name="updateId" value="<%=mp1.getId() %>">
                    	<input type="hidden" name="updateName" value="<%=mp1.getmName() %>">
                    	<input type="hidden" name="updateSid" value="<%=mp1.getsId() %>">
                    	<input type="hidden" name="updateDate" value="<%=mp1.getDate() %>">
                    	<input type="hidden" name="updateAmount" value="<%=mp1.getAmount() %>">
  
                    	<button  type="submit" value="save" rel="tooltip" class="btn btn-success btn-round btn-just-icon btn-sm" name="btnEdit">
                          <i class="material-icons" style="color : white;">edit</i>
                     </button>
                     </form>
       		 <form action = "DeleteMPurchaseServlet" method = "post">
      			  <input type = "hidden" name ="deleteItem" value ="<%=mp1.getId()%>">
